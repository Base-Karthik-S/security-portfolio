...
This README covers the **lab foundation** (Weeks 1–2): a Windows Active Directory
domain shipping Windows Security + Sysmon telemetry into an Elastic SIEM, with proof
that I can see my own activity land as searchable events.
...
Two VMs on a single 16 GB laptop, connected over VMware's **VMnet8 NAT** network
(`192.168.64.0/24`, gateway `.2`). A third host (CLIENT01) is planned for later weeks.
...
| **CLIENT01** | Windows 11 Enterprise (eval) | TBD | Domain-joined client | Pending (Week 3+) |
...
## How telemetry flows (DC01 → Fleet → Elasticsearch → Kibana)
...
The `dc01-windows` policy carries two integrations:
...
│   ├── bring-up.sh
│   └── verify.sh
├── detections/                   # one file per technique (Sigma preferred)
